...
def install_mpv():
    ...
    if install_commands:
        ...
        try:
            choice = input("\nType 'yes' to proceed with automatic installation, or press Enter to skip: ").strip().lower()
...
def download_series():
    ...
    # Build command
    command = [
        sys.executable, '-m', 'moviebox_api', 'download-series', 
        title, '-s', season, '-e', episode, '-Y'
    ]
...
def stream_series():
    ...
    command = [
        sys.executable, '-m', 'moviebox_api', 'download-series',
        title, '-s', season, '-e', episode, '--stream', '-Y'
    ]
